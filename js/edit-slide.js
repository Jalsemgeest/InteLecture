...
function setAllTextAreas() {
	...
	for (var i = 1; i < slides.length+1; i++) {
		var content = document.getElementById("slide"+i+"-html-div").innerHTML;
		slides[i-1].children[0].value = content;
		document.getElementById("slide"+i+"-html-div").innerHTML = "<p id='slide"+i+"-html'></p>";
		document.getElementById("slide"+i+"-html").innerHTML = content;
		slides[i-1].children[0].onkeyup = setHTML;
	}
}

function setAllQuestionButtons() {
	var slides = document.getElementById("slides").children;
	for (var i = 1; i < slides.length+1; i++) {
		slides[i-1].children[3].children[0].onclick = addQuestions;
	}
}
...
function addSlide() {
	var number = document.getElementById("slides").children.length+1;
	...
	document.getElementById('slide'+number).onkeyup = setHTML;
	document.getElementById('slide'+number+'-question').onclick = addQuestions;
}
...
function submitLecture() {
	...
	var lectureId = document.getElementById("lectureId").getAttribute("name");
	var slide = '{ "old_lecture_id":"'+lectureId+'", "lecture_name":"'+lectureName+'", "slide_num":"'+slide_num+'", "slides" : [';
        ...
        if (xmlhttp.readyState == 4 ) {
           if(xmlhttp.status == 200){
				if (xmlhttp.responseText == "<SUCCESS>") {
					...
					setTimeout(function() {
						document.location.href="http://54.68.82.18/0jaca/Inte-Lecture/edit.php";
					}, 8000);
				} else {
	...
	xmlhttp.open("POST", "update-exec.php", true);
