<?php
	...
	// Getting the old lectureId so it can be deleted.
	$oldLectureId = checkForSingleQuote($slides['old_lecture_id']);
	...
	// DELETE OLD ONE.
	$qry = "DELETE FROM lecture WHERE idlecture = '".$oldLectureId."' AND iduser = '".$_SESSION['SESS_USER_ID']."';";
	$result = mysql_query($qry);
	if (!$result) {
		die("Fatal error.");
	}
	
	$qry = "DELETE FROM lectureInfo WHERE idlecture = '".$oldLectureId."';";
	$result = mysql_query($qry);
	if (!$result) {
		die("Fatal error.");
	}
	
	$qry = "DELETE FROM questions WHERE idlecture = '".$oldLectureId."';";
	$result = mysql_query($qry);
	if (!$result) {
		die("Fatal error.");
	}
	
	$qry = "DELETE FROM sessions WHERE idlecture = '".$oldLectureId."';";
	$result = mysql_query($qry);
	if (!$result) {
		//die("Fatal error.");
	}
